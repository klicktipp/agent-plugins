...
| Anlegen | `add-email-editor-image-block` |
| --- | --- |
| Ändern | `update-email-editor-image-block` |
...
Die URL muss eine dieses Kontos sein. Woher sie kommt, in dieser Reihenfolge: `list-email-editor-images` (die Mediathek des Kontos — es **listet auf und sucht nicht**: eine Seite je Aufruf, weiter über `nextCursor`), sonst `search-email-editor-stock-images` (Pexels und Pixabay) und das gewählte Foto mit `sourceUrl` und `fileName` durch `open-email-editor-image-upload` — **die Provider-URL selbst gehört nie in den Newsletter**. Eigenes Material lädst du direkt mit `open-email-editor-image-upload` hoch.

Vor dem Einsetzen lohnt ein Blick: `preview-email-editor-image` zeigt eine dieser URLs als Bild im App-Fenster — Mediathek-CDN und Stock-Archive, mehr lässt die Sandbox nicht zu.
